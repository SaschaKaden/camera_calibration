...
if __name__ == '__main__':
    if CALIB_INTRINSIC:
        ...
        for i in range(0, 6):
            calib_images.append(cv2.imread(
                "data/intrinsic/calib_img{}.png".format(i), cv2.IMREAD_GRAYSCALE))
    ...
    if CALIB_HAND_EYE:
        ...
        for i in range(start_img, end_img + 1):
            ...
            transform = calib.calibrate_extrinsic(
        ...
        base_to_tcp_Ts = []
        for i in range(start_img, end_img + 1):
            base_to_ee, ee_to_base = util.load_transforms(
                "data/eye-to-hand/{}.xml".format(i))
            tcp_to_base_Ts.append(ee_to_base)
            base_to_tcp_Ts.append(base_to_ee)
        ...
        tcp_to_cam = calib.calib_hand_eye(tcp_to_base_Ts, pattern_to_cam_Ts,  method=cv2.CALIB_HAND_EYE_PARK,
                                          base_to_tcp_Ts=base_to_tcp_Ts, cam_to_pattern_Ts=cam_to_pattern_Ts)
        tcp_to_cam = calib.calib_hand_eye(tcp_to_base_Ts, pattern_to_cam_Ts,  method=cv2.CALIB_HAND_EYE_HORAUD,
                                          base_to_tcp_Ts=base_to_tcp_Ts, cam_to_pattern_Ts=cam_to_pattern_Ts)
        tcp_to_cam = calib.calib_hand_eye(tcp_to_base_Ts, pattern_to_cam_Ts,  method=cv2.CALIB_HAND_EYE_ANDREFF,
                                          base_to_tcp_Ts=base_to_tcp_Ts, cam_to_pattern_Ts=cam_to_pattern_Ts)
        tcp_to_cam = calib.calib_hand_eye(tcp_to_base_Ts, pattern_to_cam_Ts,  method=cv2.CALIB_HAND_EYE_DANIILIDIS,
                                          base_to_tcp_Ts=base_to_tcp_Ts, cam_to_pattern_Ts=cam_to_pattern_Ts)
        tcp_to_cam = calib.calib_hand_eye(tcp_to_base_Ts, pattern_to_cam_Ts,  method=cv2.CALIB_HAND_EYE_TSAI,
                                          base_to_tcp_Ts=base_to_tcp_Ts, cam_to_pattern_Ts=cam_to_pattern_Ts)

        new_board_Ts = []
        for i in range(board_Ts.__len__()):
            new_board_Ts.append(
                base_to_tcp_Ts[i] @ tcp_to_cam @ cam_to_pattern_Ts[i])
